			...
			for i in range(num):
				if connect:
					...
					if search_term.search(to_search):
						...
						if len(row.index) > 0:
							with open("../belege/einbildungskraft/belege_with_column/Einbildungs-/"+t.split(".")[0]+"_"+elem_id.text+"_"+str(left)+".item","w",encoding="utf-8") as f:
								f.write(year)
								f.write("\n")
								f.write(vol)
								f.write("\n")
							...
							with open("../belege/einbildungskraft/belege_einbildungs-.tsv","a",encoding="utf-8") as fo:
								fo.write(t.split(".")[0])
								fo.write("\t")
								...
								fo.write(url)
								fo.write("\n")
				if inpage[i].endswith('Einbildungs-'):
					connect = True
				else:
					connect = False
print(count_beleg)